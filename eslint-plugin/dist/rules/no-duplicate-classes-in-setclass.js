...
const rule = {
    ...
    create(context) {
        function checkForDuplicates(node, value, nodeStart) {
            const classes = value.split(/\s+/).filter(Boolean);
            const seen = new Map(); // class -> first occurrence index
            let currentIndex = 0;
        ...
        return {
            CallExpression(node) {
                if (node.callee.type !== "MemberExpression")
                    return;
                if (node.callee.property.type !== "Identifier" || node.callee.property.name !== "setClass")
                    return;
