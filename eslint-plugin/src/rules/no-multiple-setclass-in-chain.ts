...
const rule: Rule.RuleModule = {
  meta: {
    ...
    docs: {
      description:
        "Disallow multiple setClass() calls in a method chain (earlier calls get overwritten)",
      category: "Possible Errors",
      recommended: true,
    },
    messages: {
      multipleSetClass:
        "Multiple setClass() calls in chain. The first setClass('{{firstClasses}}') will be overwritten by the second. Use .setClass('{{firstClasses}} {{secondClasses}}') or .setClass(...).addClass(...) instead.",
    },
    schema: [],
  },
