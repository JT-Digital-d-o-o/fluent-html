...
describe("Variadic Children", () => {
  ...
  it("Variadic matches array form", () => {
    assert.strictEqual(render(render(Div(H1("A"), P("B"))) === render(Div([H1("A"), P("B")])) ? Span("match") : Span("mismatch")),
    `<span>match</span>`);
  });
  ...
  it("Variadic Section with mixed content", () => {
    assert.strictEqual(render(Section(
      H2("Section Title"),
      P("Description"),
      Div(
        Button("Action 1"),
...
describe("when() and apply()", () => {
  ...
  it("when()/apply() accept a void-returning modifier", () => {
    const sideEffect = (t: Tag): void => { t.addClass("x"); };
