...
describe("vocab validity oracle — every emission compiles in Tailwind", () => {
  ...
  for (const def of classVocab) {
    ...
    const lists: Array<{ label: string; list: readonly string[] }> = [];
    if (def.values?.kind === "literals") lists.push({ label: def.method, list: def.values.list });
    if (def.values?.kind === "group") {
      for (const [name, spec] of Object.entries(def.values.groups)) {
        if (spec.kind === "literals") lists.push({ label: `${def.method}.${name}`, list: spec.list });
      }
    }
    for (const { label, list } of lists) {
      it(`${label} literals all compile (${list.length} members)`, () => {
        for (const member of list) {
          const tuples: (readonly string[])[] = [[member]];
          if (def.emit.kind === "spacing") tuples.push(["x", member], ["y", member]);
          for (const args of tuples) {
            for (const cls of emitClasses(def.emit, args)) {
              assert.notEqual(
                design.candidatesToCss([cls])[0],
                null,
                `.${def.method}(${JSON.stringify(args)}) emitted "${cls}", which Tailwind does not compile — fix the values.literals list`,
              );
            }
          }
        }
