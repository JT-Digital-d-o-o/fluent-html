...
import { applyEdits, collectEdits, type Skip } from "../scripts/codemod/canonical-names.js";
...
function makeWorld(tagSource: string): (code: string) => Result {
...
function skipSummaries(skips: readonly Skip[]): { name: string; reason: string }[] {
  return skips
...
describe("canonical-names codemod: false-positive guards", () => {
  ...
  it("map-name calls on a non-Tag receiver are left untouched and reported", () => {
    const src =
      "class QueryBuilder { padding(_v: string): this { return this; } display(_v: string): this { return this; } }\n" +
  ...
  it("stdlib .on()/.at() on cleanly-typed receivers is silent, not SKIP noise", () => {
    const src =
      "class Emitter { on(_e: string, _cb: () => void): this { return this; } }\n" +
      'new Emitter().on("exit", () => {});\n[1, 2].at(0);\n"abc".at(1);\n';
    const { out, skips } = migrate(src);
    assert.equal(out, src);
    assert.equal(skips.length, 0);
...
describe("canonical-names codemod: transitive receiver check (post-rename lib)", () => {
  ...
  it("cross-file: collect-before-apply keeps imported components typed (pre-rename lib)", () => {
    ...
    const a2 = project.createSourceFile(
      "/a2-components.ts",
      'import { Div } from "./lib/tag.js";\nexport function Chip() { return Div().padding("2"); }\n',
    );
    const b2 = project.createSourceFile(
      "/b2-view.ts",
      'import { Chip } from "./a2-components.js";\nChip().padding("4");\n',
    );
