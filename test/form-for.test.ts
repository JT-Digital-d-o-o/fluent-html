...
describe("Form<T> binding", () => {
  it("typed input with name (no state)", () => {
    assert.strictEqual(
      ...
      `<form><input id="email" type="email" name="email"></form>`,
    );
  });
  ...
  it("input without an explicit type", () => {
    assert.strictEqual(
      ...
      `<form><input id="name" name="name"></form>`,
    );
  });
  ...
  it("select builds options and marks the wired value selected", () => {
    ...
    assert.strictEqual(
      html,
      `<form><select id="role" name="role"><option value="admin">Admin</option>\n<option value="viewer" selected>Viewer</option></select></form>`,
    );
  });
  ...
  it("label() binds `for` to the control's default id — no id/name/for repetition", () => {
    const html = render(Form<CreateUserReq>((f) => [f.label("email", "Email"), f.input("email", "email")]));
    assert.strictEqual(
      html,
  ...
  it("idPrefix namespaces control id, label for, and error aria-describedby", () => {
    const html = render(Form<CreateUserReq>({ idPrefix: "signup", errors: { email: "Required" } }, (f) => [
      f.label("email", "Email"),
      f.input("email", "email"),
      f.error("email"),
    ]));
  ...
  it(".multipart() sets the enctype", () => {
    assert.strictEqual(
      ...
      `<form action="/u" enctype="multipart/form-data"><input id="name" name="name"></form>`,
    );
  });
  ...
  it("setCapture renders the capture attribute", () => {
    assert.strictEqual(render(Form<CreateUserReq>((f) => f.input("name", "file").setCapture("user"))), `<form><input id="name" type="file" name="name" capture="user"></form>`);
  });
...
describe("Form<T> checkbox / radio binding (F-C-140)", () => {
  ...
  it("checkbox without state is unchecked", () => {
    assert.strictEqual(render(Form<SettingsReq>((f) => f.checkbox("notify"))), `<form><input id="notify" type="checkbox" name="notify"></form>`);
  });
  ...
  it("radio is checked when the field matches its value; each option gets a unique id", () => {
    ...
    assert.strictEqual(
      html,
      `<form><input id="role-admin" type="radio" name="role" value="admin" checked>\n<input id="role-viewer" type="radio" name="role" value="viewer"></form>`,
    );
  });
...
describe("Form<T> aria-invalid wiring (F-C-143)", () => {
  it("an errored input gets aria-invalid + aria-describedby, and error() id-links it", () => {
    ...
    assert.strictEqual(
      html,
      `<form><input id="email" type="email" name="email" aria-invalid="true" aria-describedby="email-error">\n<span id="email-error">Required</span></form>`,
    );
  });
