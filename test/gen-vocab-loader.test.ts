...
describe("gen-vocab design-system loader", () => {
  it("package.json pins tailwindcss to exactly the loader's version (no range)", () => {
    ...
    assert.equal(
      ...
      "devDependencies.tailwindcss must be the exact pin — no ^/~ range (the API is __unstable__)",
    );
  });
  ...
  it("loads deterministically (two loads agree on the registry)", async () => {
    ...
    assert.deepEqual(
      a.design.utilities.keys("functional").sort(),
      b.design.utilities.keys("functional").sort(),
    );
