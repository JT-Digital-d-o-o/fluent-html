...
type IgnoredRoot = { readonly prefix: string; readonly reason: string };
...
const IGNORED_ROOTS: readonly IgnoredRoot[] = [
  ...
  { prefix: "divide", reason: "backlog: bare divide color/style (divide-x/-y widths are covered)" },
  ...
  { prefix: "table", reason: "backlog: display table-* + table-layout" },
  ...
  { prefix: "invisible", reason: "backlog: visibility hidden" },
...
const matchesIgnore = (root: string): boolean =>
  IGNORED_ROOTS.some((e) => root === e.prefix || root.startsWith(`${e.prefix}-`));
...
describe("vocab coverage watch — Tailwind roots vs vocab", () => {
  ...
  it("every uncovered root is consciously ignored", () => {
    ...
    assert.deepEqual(
      unaccounted,
      [],
      `new Tailwind utility roots without a vocab row — add a row or an IGNORED_ROOTS entry with a reason: ${unaccounted.join(", ")}`,
    );
  });

  it("no stale ignore entries", () => {
    const stale = IGNORED_ROOTS.filter(
      (e) => !uncovered.some((r) => r === e.prefix || r.startsWith(`${e.prefix}-`)),
    ).map((e) => e.prefix);
    assert.deepEqual(
      stale,
      [],
      `IGNORED_ROOTS entries that no longer match any uncovered root (vocab row added, or root removed upstream): ${stale.join(", ")}`,
    );
  });
});
