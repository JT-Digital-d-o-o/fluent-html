...
describe("v4 variants (C-06): widened states + container queries", () => {
  ...
  it(".cssProp() emits the arbitrary-property class and composes with variants", () => {
    ...
    assert.strictEqual(
      render(Div().on("hover", t => t.cssProp("--glow", "0 0 4px"))),
      '<div class="hover:[--glow:0_0_4px]"></div>',
    );
    assert.strictEqual(
      render(Div().at("md", t => t.cssProp("scrollbar-width", "none"))),
  ...
  it(".on(before) + bare .content() renders the empty-string content class", () => {
    ...
    assert.strictEqual(
      render(Div().on("before", t => t.content().bg("red-500"))),
...
describe("Variant Proxy - Nesting", () => {
  ...
  it("complex nesting dark + hover + base", () => {
    assert.strictEqual(
      render(Div().bg("white").on("dark", t => t
        .bg("gray-900")
        .on("hover", t => t.bg("gray-800"))
      )),
