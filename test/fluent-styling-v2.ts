...
describe("Text Decoration", () => {
  ...
  it("underline + noUnderline on hover", () => {
    assert.strictEqual(
      render(new Tag("a").underline().hover({ noUnderline: true })),
...
describe("v4 variants (C-06): widened states + container queries", () => {
  ...
  it(".cssProp() emits the arbitrary-property class and composes with variants", () => {
    ...
    assert.strictEqual(
      render(Div().hover({ cssProp: ["--glow", "0 0 4px"] })),
      '<div class="hover:[--glow:0_0_4px]"></div>',
    );
    assert.strictEqual(
  ...
  it(".cssClass() appends verbatim (non-Tailwind intent marker)", () => {
    ...
    assert.strictEqual(
      render(Div().p("4").cssClass("shepherd-target")),
      '<div class="p-4 shepherd-target"></div>',
    );
  });
  it(".before({ content: true }) renders the empty-string content class", () => {
    ...
    assert.strictEqual(
      render(Div().before({ content: true, bg: "red-500" })),
...
describe("Object variants — states", () => {
  ...
  it("focus — two ring families chain as two calls", () => {
    ...
    assert.strictEqual(
      render(Button().focus({ ring: 2, outline: "none" }).focus({ ring: "blue-300" })),
      '<button class="focus:ring-2 focus:outline-none focus:ring-blue-300"></button>',
    );
  });
  it("disabled", () => {
  ...
  it("false and undefined values are skipped", () => {
    assert.strictEqual(
      render(Div().hover({ truncate: false, bg: undefined, opacity: "50" })),
      '<div class="hover:opacity-50"></div>',
    );
  });
  it("conditional value expressions replace in-variant conditionals", () => {
    const active = false;
    assert.strictEqual(
      render(Div().hover({ bg: active ? "blue-600" : undefined, italic: !active })),
      '<div class="hover:italic"></div>',
    );
  });
});
...
describe("Object variants — breakpoints", () => {
  ...
  it("responsive chain", () => {
    assert.strictEqual(
      render(Div().w("full").md({ w: "1/2" }).lg({ w: "1/3" })),
...
describe("Object variants — nesting", () => {
  ...
  it("complex nesting dark + hover + base", () => {
    assert.strictEqual(
      render(Div().bg("white").dark({ bg: "gray-900", hover: { bg: "gray-800" } })),
      '<div class="bg-white dark:bg-gray-900 dark:hover:bg-gray-800"></div>'
    );
  });
  it("two-level nesting md:hover:first", () => {
    assert.strictEqual(
      render(Div().md({ hover: { first: { bg: "amber-50" } } })),
      '<div class="md:hover:first:bg-amber-50"></div>'
    );
  });
});
...
describe("Object variants — integration", () => {
  ...
  it("object variant with .when()", () => {
    assert.strictEqual(
      render(
        Button()
          .hover({ bg: "blue-600" })
          .when(true, t => t.disabled({ opacity: 50 }))
      ),
  ...
  it("object variant skipped with .when(false)", () => {
    assert.strictEqual(
      render(
        Button()
          .hover({ bg: "blue-600" })
          .when(false, t => t.disabled({ opacity: 50 }))
      ),
  ...
  it("presets spread into variant objects", () => {
    ...
    assert.strictEqual(
      render(Div().hover({ ...glow, bg: "blue-600" })),
      '<div class="hover:shadow-lg hover:ring-2 hover:bg-blue-600"></div>'
    );
  });
...
describe("Full Example (Plan Before/After)", () => {
  it("complete button with all features", () => {
    assert.strictEqual(
      ...
      '<button class="px-4 py-2 bg-blue-500 text-white rounded transition duration-200 hover:bg-blue-600 hover:scale-105 hover:shadow-lg focus:ring-2 focus:outline-none focus:ring-blue-300 disabled:opacity-50 disabled:cursor-not-allowed md:px-8 md:text-lg"></button>'
    );
  });
});
...
describe("Variant exception-safety (D-06)", () => {
  it("an unknown style key throws and does not leak the variant prefix onto later classes", () => {
    ...
    assert.throws(
      () => tag.hover({ bg: "blue-600", bogus: "x" } as unknown as VariantStyleObject),
      /unknown style key "bogus"/,
    );
  ...
  it("a throw inside .variant(...) does not leak the breakpoint prefix", () => {
    const tag = Span();
    assert.throws(() => tag.variant("md", { nope: true } as unknown as VariantStyleObject));
    tag.text("black");
