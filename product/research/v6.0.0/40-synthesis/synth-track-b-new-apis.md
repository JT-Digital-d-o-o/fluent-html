...
> ⚠️ **Heavily superseded by [`curation.md`](./curation.md) §4–§5 + [`v6-spec.md`](./v6-spec.md).** The "configure once via context, compose everywhere" thesis is **reversed**: fluent-html is an **instruction set** (§4) — core keeps only `Form<T>(state, f=>…)`, dialog behaviors (`openDialog`/`closeDialog`), `.gradient()`, full SVG coverage, and `.htmxIndicator()`. **Everything else is CUT to user-land** (`@jtdigital/ui`: Alert/Badge/Card/Modal/Drawer/Toast/Field/Icon/Table-grid/Shell/SeoHead) or **moved to the framework** (`@fluent-html/fastify`: B-07 render adapter, B-08 `renderView(opts)`/`Deferred`, B-09 request context). Theming converges to **one `defineTheme()`**. Read for evidence only.
