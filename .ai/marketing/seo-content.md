...
```typescript
// The template emits title, description, Open Graph, Twitter card, and a
// self-referencing canonical from Layout's `seo` prop — never hand-emit Meta/Link tags:
Layout({
  title: "Page Title — Primary Keyword",   // <60 chars; site name is appended automatically
  seo: {
    description: "150-160 char description with target keyword",
    // ogImage?: custom social card (defaults to /og-image.png)
...
### DO: Register public pages in the sitemap
```typescript
// The template serves /sitemap.xml from the server.sitemap registry — never a
// static file. Each controller registers its public GET routes:
server.sitemap.add(pricingRoutes.index);                          // static page
server.sitemap.addDynamic(blogRoutes.post, async () =>            // per-entity pages, with lastmod
  (await db.post.findMany()).map((p) => ({ params: { slug: p.slug }, lastmod: p.updatedAt })));

// A CI drift audit fails if a public GET route is neither added nor excluded.
// Submit the sitemap to Google Search Console after launch.
```

### DON'T: Block crawlers from important pages
```typescript
// robots.txt is generated by the seo controller — don't hand-edit Disallow lines.
// Keep pages out of search with sitemap.exclude, which also stamps X-Robots-Tag: noindex:
server.sitemap.exclude(authRoutes.login, shareRoutes.view);  // ✓ reachable, not search material
server.sitemap.exclude(pricingRoutes.index);                 // ✗ never exclude money pages

// Guarded routes (preHandler auth) are automatically out of sitemap scope.
```
