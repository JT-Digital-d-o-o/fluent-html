...
guideline_updates:
  - "README.md (lib) — new 'Declarative Shadow DOM & web-component theming' section: Template().setShadowrootmode(\"open\") + the three DSD boolean tokens; scoped <style> with :host/::slotted/::part; the setSlot (sender) vs Slot/setName (receiver) triad; setPart/addPart/setExportparts ::part theming (canonical no-space exportparts grammar)"
  - "fluent-html.md — web-components reference rows: setShadowrootmode, the 3 shadowroot* boolean tokens (global on BooleanAttribute), setSlot, setPart/addPart, setExportparts; the SSR-DSD/htmx setHTMLUnsafe caveat"
  - "JSDoc — TemplateTag.setShadowrootmode (open/closed distinction); the 3 new BooleanAttribute members documented in html-types.ts; Tag.setSlot (sender vs SlotTag.setName receiver; '' emits slot=\"\", only undefined clears); Tag.setPart/addPart (only effective inside a shadow tree); Tag.setExportparts (host-only: inert on a non-host element; canonical no-space colon grammar)"
...
// They are GLOBAL members of BooleanAttribute (type-valid via Tag.toggle on any
// tag, exactly like 'selected'/'checked'/'open'); set via Tag.toggle — no new
// method, reuses the bare-boolean serialize path:
 ...
 * `<slot name="header">`. Only effective when an ancestor is a shadow host
 * (custom element or DSD `<template>`); otherwise inert per standard HTML.
 *
 * `""` vs `undefined` is intentional and asymmetric: `setSlot("")` emits
 * `slot=""` — the valid "assign to the default (unnamed) slot" case — because
 * the clear-guard is `name === undefined`. `setSlot()` / `setSlot(undefined)`
 * clears the attribute (no `slot` emitted), mirroring every other
 * `undefined`-clearing setter.
 *
 * @example H1("Title").setSlot("header")   // <h1 slot="header">Title</h1>
 * @example Div("x").setSlot("")            // <div slot="">x</div>  (default slot)
 * @example Div("x").setSlot()              // <div>x</div>          (cleared)
 */
 ...
 * tree; inert on a non-host/non-shadow element per standard HTML.
 *
 ...
 * counterpart of `setPart`. `addPart("")` is a no-op. Only effective inside a
 * shadow tree; inert on a non-host/non-shadow element per standard HTML.
 *
 ...
 * the canonical no-space `"inner:exposed"` (the MDN/spec form).
 *
 * `exportparts` is valid **only on a shadow host** (a custom element or a DSD
 * `<template>` host) — a strictly narrower set than `part`/`slot`. It lives on
 * global `Tag` as a deliberate, `BooleanAttribute`-style global-union tradeoff
 * (no per-element narrowing exists today); it is **inert on a non-host element,
 * per standard HTML**.
 *
...
| `Div("x").setSlot("")` | `<div slot="">x</div>` (default slot) |
...
- **`setExportparts` types the *structure*, not the token names.**
  ...
  makes the rename arm explicit; `["a","b","c"]` is a compile error (TS2345,
  verified against repo-local `tsc`), and the bare 2-tuple literal compiles
  *without* `as const` (contextually typed). `part`/`slot`/`exportparts` *names*
  are author-defined idents matched against author-written CSS/markup — there is
  **no closed platform vocabulary** to constrain them to, so `string` is correct
  here (the §11.4 bare-string ban targets cases where a closed union *exists*,
  e.g. `ShadowRootMode`; same justification as `setLang`'s bare `string`).
...
1. **`README.md` (lib root)** — new **"Declarative Shadow DOM & web-component
   ...
   nested parts with `.setExportparts("label", ["icon", "lead-icon"])` (emits the
   canonical no-space `exportparts="label, icon:lead-icon"`).
   ```

2. **`fluent-html.md`** — web-components reference rows: `setShadowrootmode`; the
   three `shadowroot*` boolean tokens (via `.toggle()`, **global on
   `BooleanAttribute`**); `setSlot` (note: sender, contrast `SlotTag.setName`
   receiver; `""` → `slot=""`, `undefined` clears); `setPart`/`addPart`
   (set=override, add=accumulate); `setExportparts` (note the canonical
   `", "` map separator / no-space `":"` rename grammar; host-only). Add the
...
3. **JSDoc** — drafted in *Proposed API*: `setShadowrootmode` (open/closed
   ...
   (documented as global tokens); `setSlot` (sender vs `SlotTag.setName`
   receiver; `""` vs `undefined` asymmetry); `setPart`/`addPart` (only effective
   inside a shadow tree); `setExportparts` (host-only / inert on non-host;
   canonical no-space colon grammar).
...
4. **`CHANGELOG.md`** — "Added" entry under 6.2.0: `TemplateTag` +
   ...
   (`Tag`→`TemplateTag`, additive). **Explicitly call out** that the three
   `shadowroot*` boolean tokens are **global on `BooleanAttribute`** (type-valid
   via `.toggle()` on any tag, exactly like `selected`/`checked`), so the
   documented global-union tradeoff is on the record, not just in the RFC.
...
- **§11.3 escape-by-default** — `slot`/`part`/`exportparts` flow through `addAttribute` → `escapeAttr` at render (`tag.ts:188`/`serialize.ts:285`); `shadowrootmode` is a closed enum on the schema render path (`serialize.ts:270`); the `shadowroot*` booleans are bare keywords (no value) gated by `BOOLEAN_ATTR_RE`. No new XSS sink.
- **§11.4 type-safety** — `ShadowRootMode` and the three boolean tokens are closed unions (a typo is a compile error); `setExportparts` types the tuple structure (1/3-tuple are compile errors, 2-tuple compiles without `as const` — verified against repo-local `tsc`); `string` only for author-defined ident names where no closed platform vocabulary exists (same posture as `setLang`).
...
- **§11.8 docs/guideline-sync** — lib README (new DSD/theming section) + `fluent-html.md` rows + JSDoc on every new symbol + CHANGELOG (with the global-boolean callout); plus a one-line attribute-only note in both tooling READMEs; the corrected no-space `exportparts` grammar propagates to README/`fluent-html.md`/JSDoc so documented output matches the emit. Covers every symbol in `api_surface`.
...
- **Per-element narrowing so `shadowroot*` booleans / `part`/`exportparts` only
  type-check on the right element.** Rejected for this RFC: `BooleanAttribute` is
  a *global* union by design (`'selected'`/`'checked'` already live there despite
  being element-specific); narrowing is a larger breaking redesign out of scope.
- **Emit the spaced `"inner: exposed"` exportparts rename grammar.** Rejected:
  although the W3C CSS Shadow Parts L1 parser trims whitespace around the colon
  (so it parses correctly), MDN documents the canonical form as `inner:exposed`
  with **no** space. The spaced variant is a needless deviation that surprises
  anyone diffing against canonical/MDN output; the emit uses the canonical
  no-space `:`.
...
3. **`exportparts` grammar.** Resolved: emit the **canonical** separators —
   `", "` between maps, no-space `":"` inside a rename (matching MDN/spec). A
   unit test must assert the exact string `"card, inner-label:label"`. We leave
   token-shape validation out and keep raw string concat (consistent with the
   rest of the surface).
...
Adversary verdict: **survives-with-changes** (confidence 0.78). All four
`required_changes` folded into the contract above:

1. **`setExportparts` canonical no-space colon grammar.** Changed the emit from
   `${m[0]}: ${m[1]}` to `${m[0]}:${m[1]}`, so a rename map emits the canonical
   MDN/spec `inner:exposed` (not `inner: exposed`); the `", "` map separator is
   retained (canonical). Propagated to **all four** locations in lockstep: the
   impl in *§`src/core/tag.ts`*, the JSDoc `@example`
   (`exportparts="card, inner-label:label"`), the *Emitted output* table row, and
   *Open question 3*'s asserted unit-test string (`"card, inner-label:label"`,
   no-space form). Also recorded in *Alternatives considered* and the README/docs
   examples. **Resolved.**

2. **`setSlot` empty-string semantics.** Added a JSDoc note (and a *Emitted
   output* table row) making the `""` vs `undefined` asymmetry explicit:
   `setSlot("")` emits `slot=""` (the valid assign-to-default/unnamed-slot case),
   while `setSlot()` / `setSlot(undefined)` clears — the guard is
   `name === undefined`. **Resolved.**

3. **`setExportparts` host-only caveat.** Added the explicit "inert on a non-host
   element, per standard HTML" caveat to the `setExportparts` JSDoc (matching the
   caveats on `setSlot`/`setPart`), documenting the global-`Tag` breadth as a
   deliberate `BooleanAttribute`-style global-union tradeoff (`exportparts` is
   valid only on a shadow host — narrower than `part`/`slot`), not an oversight.
   **Resolved.**

4. **CHANGELOG global-boolean callout.** The *Docs impact* CHANGELOG item now
   explicitly states the three `shadowroot*` boolean tokens are **global on
   `BooleanAttribute`** (type-valid via `.toggle()` on any tag, like
   `selected`/`checked`), putting the documented tradeoff on the record. The
   `html-types.ts` comment and `fluent-html.md` row carry the same note.
   **Resolved.**

No required change was declined. The verdict's confirmations (not-shipped, no
naming collisions, verified type story, escape paths intact, §11.7 N/A, breaking
honestly marked, architecture fit) require no contract change and are reflected
unchanged.
