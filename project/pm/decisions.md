...
## Improve fluent styling for LLM authors — typed-string migration rejected
**Date:** 31. 07. 26
**Context:** With Claude (Opus/Fable class) as the code author, the styling surface was re-evaluated: could fluent methods be replaced by compile-time-validated Tailwind class strings (template-literal validator, variants in-string, runtime merge for overrides)? A ~9-agent study spiked the validator (works: per-token validation, error accumulation, theme augmentation, ~1s fixed tsc cost), measured token economy (strings = 0.63× fluent), and scored both systems across 12 dimensions.
**Decision:** Reject the string migration; improve fluent instead via five tracks (scope: [llm-styling/](llm-styling/)): single-source vocab generation with a Tailwind design-system validity oracle, escape-hatch closure (typed `.cssProp`/`.cssClass` + CI-blocking lint), canonical method names (method = class prefix, 17 spike-proven merges), object-form variants (`.hover({…})` replacing `.on()`/`.at()`), and an optional variant-free append-only `.tl()` sink gated on a post-release decision.
**Reasoning:** Improved fluent matches the string migration's aggregate score (~77–78 vs ~73) while beating it decisively on silent-error resistance (8 vs 4): in-string variants make dropped `hover:`/`md:` prefixes compile-clean-but-wrong, and any runtime merge table can silently delete a written class — failure modes with no fluent equivalent. The string form's real advantages (token economy, training-prior alignment, loud batch-repairable errors) are mostly capturable by renames + object variants + lint, at ~5–6 weeks vs 6–10 and with every phase reversible.
**Consequences:** No runtime class merger ever (append-only stays); canonical-names and object-variants must ship as one breaking release; `.tl()` if adopted is variant-free, append-only, and lint-ratcheted — it is not a general string API. The PM scope was created eagerly (full grid upfront) by explicit maintainer choice, deviating from lazy birth.

## Object-variants `2xl` member is spelled `xl2` (method and nested key); `.variant("2xl", …)` keeps the exact Tailwind spelling
**Date:** 01. 08. 26
**Context:** Object variants (llm-styling/object-variants) give every tier-1 variant a direct method (`.hover({…})`, `.md({…})`) and a nested-object key (`md: { hover: {…} }`). Tailwind's largest default breakpoint is `2xl`, which is not a valid JS identifier — a direct method cannot be named `.2xl()`, and an object key would need quotes (`{ "2xl": {…} }`). The spike used `xl2`; final naming was left as a scope decision.
**Decision:** The tier-1 member is **`xl2`** in both positions — `.xl2({…})` and `{ xl2: {…} }` — emitting the `2xl:` prefix. The generic `.variant("2xl", {…})` form (whose `TailwindBreakpoint` union already contains `"2xl"`) remains the exact-spelling route, as it is for every other variant. Container-query breakpoints (`@sm`, `@max-lg`, …) stay `.variant()`-only.
**Reasoning:** A quoted `"2xl"` key would be the only quoted member of the whole surface — visually inconsistent, easy for an LLM to mis-emit unquoted (a compile error, but a needless one), and unusable as a method name, splitting the spelling across positions (`.variant("2xl")` but `md: { "2xl": … }`). `xl2` is one uniform identifier in both positions, keeps the breakpoint family prefix-sortable (`sm md lg xl xl2`), and lands in the already-documented "where names diverge from raw Tailwind" divergence family (compound-prefix boundary, `.neg()`, translate axis). Divergence cost is low: `2xl` is rare, and the generic form preserves the trained spelling.
**Consequences:** `DIRECT_VARIANTS` maps `xl2 → "2xl"`; docs teach `xl2` in the tier-1 table and note the divergence in FLUENT-STYLING's divergence section; the ESLint derived tables treat `xl2` as the owner of the `2xl:` prefix.
