...
- [x] [P0] `fluentHtmlPlugin(opts?)` — Vite plugin that **writes the safelist to a real `.css` file** (`outFile`, default `./src/fluent-safelist.css`) at `buildStart` + on dev change; the app CSS `@import`s it. **(Redesigned after e2e:** the original virtual-module approach was **broken** — `@tailwindcss/vite` resolves CSS `@import`s from disk via `enhanced-resolve`, bypassing Vite `resolveId`, so the virtual module was never seen. Verified the file approach against real Tailwind v4.3.1 + Vite 5: render-time-only classes `bg-brand`/`hover:bg-forest`/`bg-linear-to-br` + `--color-brand` all land in the bundled CSS. Dropped the PostCSS-polymorphism/`transform` machinery — non-Vite uses `generateFluentSafelist` in a prebuild.)
