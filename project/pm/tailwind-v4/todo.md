...
- [x] [P0] Build wiring = **`generateFluentSafelist` in a prebuild** writes a real `.css` file the app CSS `@import`s. **(The shipped Vite plugin was first redesigned, then REMOVED entirely:** e2e found the original virtual-module `fluentHtmlPlugin` was **broken** — `@tailwindcss/vite` resolves CSS `@import`s from disk, bypassing Vite `resolveId`. Then: **none of our apps use Vite** (Fastify SSR + PostCSS), so the whole Vite plugin was dead surface and was deleted — kept `generateFluentSafelist` + `globFiles`. Verified e2e against Tailwind v4.3.1 + **`@tailwindcss/postcss`** (the apps' real path): `bg-brand`/`hover:bg-forest`/`bg-linear-to-br` + `--color-brand` all land in the output CSS.)
...
- [x] [P0] Wired `defineTheme` tokens → `@theme` CSS + safelist via `generateFluentSafelist(files, { theme })` (extractor): `themeToCss` (family→`--color-*`/`--spacing-*`/`--text-*`/`--radius-*`/`--shadow-*`) + `themeToManifest` (token-utility safety net for variable usage), prepended to the `@source inline` block
