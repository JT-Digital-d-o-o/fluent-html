# Fluent HTML — full reference

> **This is the long-form reference** (the pre-8.0.0 package README, demoted out of the
> agent-facing head doc). Start with the [README](README.md) — the census-ranked head
> covers ~90% of real call sites in a fraction of the reading. Come here for the deep
> prose on any one subsystem. The complete callable surface with fleet call-site counts
> is in [generated/full-surface.md](generated/full-surface.md).
>
> Bare route-string arguments in the examples below predate the 8.0.0 `ResolvedRoute`
> brand — in 8.0.0 code they come from a route callable / `.resolve()`, or wrap in
> `assetUrl()` / `externalUrl()` (see the README's pattern 1 and the CHANGELOG).
...
Button("Remove").setHtmx(hx("/api/item/1", { method: "delete", confirm: "Sure?" }))
...
```typescript
// Shorthand methods — method is baked in (GET/POST; other verbs go through hx())
...
Div(
  Button("Delete 1").setHtmx(hx("/item/1", { method: "delete" })),
  Button("Delete 2").setHtmx(hx("/item/2", { method: "delete" })),
