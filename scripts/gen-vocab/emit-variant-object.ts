...
import type { LeafValuesSpec, ValuesSpec } from "../../src/class-vocab/types.js";

/** `@theme` namespace → the curated union in `tailwind-types` that represents it. */
const THEME_TYPES: Readonly<Record<string, string>> = {
...
export function variantObjectGeneratedPath(): string {
...
function unionIndex(): { byAddress: Map<string, string>; byList: { typeName: string; list: readonly string[] }[] } {
  const byAddress = new Map<string, string>();
  const byList: { typeName: string; list: readonly string[] }[] = [];
...
function leafType(spec: LeafValuesSpec, address: string): string {
  switch (spec.kind) {
    case "typeRef":
      return spec.name;
    case "theme": {
      const name = THEME_TYPES[spec.ns];
      if (!name) throw new Error(`emit-variant-object: no type mapping for theme namespace "${spec.ns}" (${address})`);
      return name;
    }
    case "literals": {
      const named = UNION_BY_ADDRESS.get(address) ?? UNION_BY_LIST.find((u) => sameList(u.list, spec.list))?.typeName;
      if (!named) throw new Error(`emit-variant-object: no template union covers the literals of "${address}" — name one or set variantObject.type`);
      return named;
    }
  }
}
...
function valuesType(values: ValuesSpec, method: string): string {
...
export function keyType(spec: VariantKeySpec): string {
  if (spec.type !== undefined) return spec.type;
  const { def, emit } = spec;
  if (emit.kind === "static") return "boolean";
  if (!def.values) {
...
function referencedTypeNames(exprs: readonly string[]): readonly string[] {
  const names = new Set<string>();
  for (const expr of exprs) {
    for (const m of expr.matchAll(/\b(Tailwind\w+|CssPropertyName)\b/g)) names.add(m[1]!);
  }
  return [...names].sort();
}

/** Render the full content of `variant-object.gen.ts`. */
export function renderVariantObjectGen(): string {
