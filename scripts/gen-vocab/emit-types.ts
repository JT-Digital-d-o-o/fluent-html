 ...
 */
export function renderUnion(typeName: string, members: readonly string[], arbitrary: boolean, extraArms: readonly string[] = []): string {
 ...
 */
export function literalsOf(address: string): readonly string[] {
  const [method, groupName, ...extra] = address.split(".");
  if (extra.length > 0) throw new Error(`gen-vocab: malformed union address "${address}" (expected method or method.group)`);
  ...
  if (groupName !== undefined) {
    ...
    if (!spec) throw new Error(`gen-vocab: vocab row "${method}" has no group "${groupName}"`);
  }
