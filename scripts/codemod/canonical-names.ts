...
function chainEntries(body: Expression, param: string): { entries?: ObjEntry[]; reason?: string } {
  ...
  for (const { name, node } of calls.reverse()) {
    if (name === "on" || name === "at") {
      ...
      if (nested.chunks.length !== 1) return { reason: "nested variant needs a repeated key" };
...
function collectEdits(file: SourceFile): { edits: Edit[]; skips: Skip[] } {
  ...
  file.forEachDescendant((node) => {
    ...
    if (!receiverIsTag(callee.getExpression())) {
      ...
      if (!inSpan(variantSpans, nameStart)) skips.push({ line, name, reason: "receiver does not type as Tag" });
      return;
    }
    if (name === "on" || name === "at") {
