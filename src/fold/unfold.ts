...
export function unfoldView<S>(coalg: ViewCoalgebra<S>, seed: S): View {
  ...
  switch (layer.type) {
    ...
    case "tag": {
      ...
      if (attrs) {
        if (attrs.id) tag.id = attrs.id;
        if (attrs.class) tag.class = attrs.class;
        if (attrs.style) tag.style = attrs.style;
        if (attrs.attributes) tag.attributes = { ...attrs.attributes };
        if (attrs.htmx) tag.htmx = attrs.htmx;
        if (attrs.toggles) tag.toggles = attrs.toggles;
      }
      return tag;
    }
    case "list":
