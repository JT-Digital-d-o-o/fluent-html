 ...
 * @internal
 */
export let devChecks: boolean = readDefault();
...
export function nextRenderEpoch(): number {
  return ++epoch;
}
 ...
 * @internal
 */
export function assertMutable(tag: { el: string; _e: number; _p: number }, method: string): void {
  if (tag._e !== 0) {
    throw new Error(
      `<${tag.el}>.${method}() mutates a tag that has already been rendered. ` +
  ...
  if (tag._p > 1) {
    throw new Error(
      `<${tag.el}>.${method}() mutates a tag that is a child of ${tag._p} parents. ` +
 ...
 * @internal
 */
export function countParents(children: readonly unknown[]): void {
  for (let i = 0; i < children.length; i++) {
    const c = children[i];
    if (c === null || c === undefined) continue;
    if (Array.isArray(c)) countParents(c);
    else if (typeof c === "object" && (c as { _t?: number })._t === 1) {
      (c as { _p: number })._p++;
    }
  }
}
