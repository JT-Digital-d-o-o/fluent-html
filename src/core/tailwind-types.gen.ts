...
import type {
  FluentColorConfig, FluentCustomColors, FluentCustomSpacing, FluentCustomFontSize, FluentCustomRadius, FluentCustomShadow,
...
// The built-in palette arm is gated behind FluentColorConfig (T2): an app that
// augments `{ defaultPalette: false }` collapses it to `never`, making every
// palette literal a compile error while tokens and keywords keep working.
type DefaultPaletteColor = FluentColorConfig extends { defaultPalette: false }
  ? never
