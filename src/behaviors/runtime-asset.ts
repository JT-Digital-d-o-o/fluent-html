...
export function behaviorRuntimeVersion(): string {
  if (cachedVersion === undefined) {
    // dist/src/behaviors/runtime-asset.js → ../../../package.json
    const pkgPath = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "..", "package.json");
    cachedVersion = (JSON.parse(readFileSync(pkgPath, "utf8")) as { version: string }).version;
  }
 ...
 */
export function behaviorStamp(): string {
 ...
 */
export function behaviorRuntimeFileName(hash?: string): string {
 ...
 */
export function behaviorRuntimeSource(): { fileName: string; source: string } {
...
export function readAssetStamp(source: string): { version: string; registryHash: string } | undefined {
 ...
 */
export function assertBehaviorRuntimeAsset(source: string): void {
  ...
  if (actual !== expected) {
    throw new Error(
      `fluent-behaviors asset mismatch: asset is ${actual}, server expects ${expected}. Rebuild the runtime asset (buildBehaviorRuntime) or align the fluent-html version.`,
    );
  }
}
