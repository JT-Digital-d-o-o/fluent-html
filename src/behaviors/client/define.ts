...
export type Wire<T> = T extends void
  ? Record<string, never>
  : { [K in keyof T]: WireValue<T[K]> };
// Non-distributive ([V] extends […]) so union-typed options map as a whole:
// `Id | readonly Id[]` (an id-list) is always decoded to string[], never string | string[].
type WireValue<V> = [V] extends [Id]
  ? WireTarget
  : [V] extends [Id | readonly Id[]]
    ? string[]
    : [V] extends [Id | "@self" | { closest: string }]
      ? WireTarget
      : V extends readonly (infer E extends string)[]
        ? E[]
        : V extends object
          ? Wire<V>
          : V;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- handlers are looked up untyped by the dispatcher
...
export const handlers: Record<string, BehaviorHandler> = Object.create(null) as Record<string, BehaviorHandler>;
...
export function defineBehavior<K extends keyof BehaviorMap>(
  name: K,
  handler: (el: Element, opts: Wire<BehaviorMap[K]>, fx: FxCtx) => boolean | void,
): void {
  if (handlers[name as string]) throw new Error(`defineBehavior: duplicate "${name as string}"`);
  handlers[name as string] = handler as BehaviorHandler;
}
