...
function emitSpacing(prefix: string, sep: "" | "-", abbrev: boolean, units: boolean, args: readonly string[]): string[] {
  if (args.length === 1) return [`${prefix}-${args[0]}`];
  if (args.length === 2) {
    const a = args[0]!;
    const b = args[1]!;
    if (units && UNITS.has(a)) return [`${prefix}-[${b}${a}]`];
...
function emitSizing(prefix: string, args: readonly string[]): string[] {
  if (args.length === 1) return [`${prefix}-${args[0]}`];
  if (args.length === 2 && UNITS.has(args[0]!)) return [`${prefix}-[${args[1]}${args[0]}]`];
  return [];
}
 ...
 */
export function emitClasses(shape: EmitShape, args: readonly string[] = []): string[] {
  switch (shape.kind) {
    case "static":
      return [shape.class];
    case "prefix":
 ...
 */
const PREFIX_BY_METHOD: Readonly<Record<string, string>> = (() => {
  const map: Record<string, string> = Object.create(null) as Record<string, string>;
  for (const def of classVocab) {
    const s = def.emit;
    switch (s.kind) {
      case "static": map[def.method] = s.class; break;
      case "prefix":
      case "optional":
      case "spacing":
      case "sizing": map[def.method] = s.prefix; break;
 ...
 */
export function prefixOf(method: string): string {
