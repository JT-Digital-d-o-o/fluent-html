...
import { render, Form, Input, Textarea, Button, Label, Select, Option, Optgroup, Datalist, Fieldset, Legend, Output, defineIds, } from "../src/index.js";
// ------------------------------------
// Forms - Input
// ------------------------------------
describe("Forms - Input", () => {
    ...
    it("Input with inputmode", () => {
        assert.strictEqual(render(Input().setType("text").setName("otp").setInputmode("numeric")), `<input type="text" name="otp" inputmode="numeric">`);
    });
    it("Number input via factory with min/max", () => {
        assert.strictEqual(render(Input("number").setName("qty").setMin(1).setMax(100).setStep(5)), `<input type="number" name="qty" min="1" max="100" step="5">`);
    });
    it("Date input with string min/max", () => {
        assert.strictEqual(render(Input("date").setName("start").setMin("2024-01-01").setMax("2024-12-31")), `<input type="date" name="start" min="2024-01-01" max="2024-12-31">`);
    });
    it("Range input with numeric min/max", () => {
        assert.strictEqual(render(Input("range").setName("volume").setMin(0).setMax(100).setStep(10)), `<input type="range" name="volume" min="0" max="100" step="10">`);
    });
    it("Time input with string min/max", () => {
        assert.strictEqual(render(Input("time").setName("alarm").setMin("09:00").setMax("17:00")), `<input type="time" name="alarm" min="09:00" max="17:00">`);
    });
