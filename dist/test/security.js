...
describe("XSS — Text Content", () => {
    it("Escapes script tags in text", () => {
        assert.strictEqual(render(Div("<script>alert('xss')</script>")), `<div>&lt;script&gt;alert(&#39;xss&#39;)&lt;/script&gt;</div>`);
    });
    it("Escapes nested injection attempts", () => {
        assert.strictEqual(render(Div(Span("<img src=x onerror=alert(1)>"))), `<div><span>&lt;img src=x onerror=alert(1)&gt;</span></div>`);
    });
...
describe("XSS — Attribute Values", () => {
    it("Escapes quotes in attribute values", () => {
        assert.strictEqual(render(Div().addAttribute("data-value", `" onmouseover="alert(1)`)), `<div data-value="&quot; onmouseover=&quot;alert(1)"></div>`);
    });
    it("Escapes angle brackets in attribute values", () => {
        assert.strictEqual(render(Div().addAttribute("data-value", `<script>alert(1)</script>`)), `<div data-value="&lt;script&gt;alert(1)&lt;/script&gt;"></div>`);
    });
    it("Escapes in class attribute", () => {
        assert.strictEqual(render(Div().setClass(`"><script>alert(1)</script>`)), `<div class="&quot;&gt;&lt;script&gt;alert(1)&lt;/script&gt;"></div>`);
    });
    it("Escapes in style attribute", () => {
        assert.strictEqual(render(Div().setStyle(`color: red" onclick="alert(1)`)), `<div style="color: red&quot; onclick=&quot;alert(1)"></div>`);
    });
    it("Escapes in id attribute", () => {
        assert.strictEqual(render(Div().setId(`foo" onclick="alert(1)`)), `<div id="foo&quot; onclick=&quot;alert(1)"></div>`);
    });
});
// ------------------------------------
...
describe("XSS — Attribute Keys", () => {
    ...
    it("Blocks onload attribute", () => {
        assert.throws(() => Div().addAttribute("onload", "alert(1)"), /Event handler attribute "onload" is blocked/);
    });
    it("Blocks onmouseover attribute", () => {
        assert.throws(() => Div().addAttribute("onmouseover", "alert(1)"), /Event handler attribute "onmouseover" is blocked/);
    });
    it("Rejects attribute key with quote (context breakout)", () => {
        assert.throws(() => Div().addAttribute('" onmouseover="x', "y"), /Invalid attribute key/);
    });
    it("Rejects attribute key with angle brackets", () => {
        assert.throws(() => Div().addAttribute("<script>", "y"), /Invalid attribute key/);
    });
    it("Rejects attribute key with spaces", () => {
        assert.throws(() => Div().addAttribute("data value", "y"), /Invalid attribute key/);
    });
    it("Allows data-custom attributes", () => {
        assert.strictEqual(render(Div().addAttribute("data-custom", "safe")), `<div data-custom="safe"></div>`);
    });
    it("Allows hx-get attributes", () => {
        assert.strictEqual(render(Div().addAttribute("hx-get", "/api")), `<div hx-get="/api"></div>`);
    });
    it("Allows aria-label attributes", () => {
        assert.strictEqual(render(Div().addAttribute("aria-label", "Close")), `<div aria-label="Close"></div>`);
    });
});
// ------------------------------------
...
describe("Script/Style Injection", () => {
    ...
    it("Preserves valid script content", () => {
        assert.strictEqual(render(Script("if (x < 10 && y > 5) { return '<tag>'; }")), "<script>if (x < 10 && y > 5) { return '<tag>'; }</script>");
    });
    it("Preserves valid style content", () => {
        assert.strictEqual(render(Style(`.class > .child { content: "a & b"; }`)), `<style>.class > .child { content: "a & b"; }</style>`);
    });
});
// ------------------------------------
// Prototype Pollution
...
describe("Prototype Pollution", () => {
    it("Blocks __proto__ as attribute key", () => {
        assert.throws(() => Div().addAttribute("__proto__", "{}"), /prototype pollution/);
    });
    it("Blocks constructor as attribute key", () => {
        assert.throws(() => Div().addAttribute("constructor", "{}"), /prototype pollution/);
    });
    it("Blocks prototype as attribute key", () => {
        assert.throws(() => Div().addAttribute("prototype", "{}"), /prototype pollution/);
    });
});
// ------------------------------------
...
describe("CSP Nonce", () => {
    it("setNonce adds nonce attribute to Script", () => {
        assert.strictEqual(render(Script("console.log('hi')").setNonce("abc123")), `<script nonce="abc123">console.log('hi')</script>`);
    });
    it("setNonce adds nonce attribute to Style", () => {
        assert.strictEqual(render(Style(".x { color: red }").setNonce("abc123")), `<style nonce="abc123">.x { color: red }</style>`);
    });
    it("renderWithNonce applies nonce to all script/style tags", () => {
        const result = renderWithNonce("nonce123", Div(Script("console.log('a')"), Style(".b { color: blue }"), P("text")));
