...
const throws = (fn, match) => assert.throws(fn, match);
// ------------------------------------
// Mutate after render
// ------------------------------------
describe("dev-checks — mutate after render", () => {
    ...
    it("a fresh tag per render is unaffected", () => {
        const view = () => Div("x").p("4").bg("red-500");
...
describe("dev-checks — aliased children", () => {
    ...
    it("a shared but unmutated child stays legal (non-thunk separator form)", () => {
        const sep = Span("/");
        assert.strictEqual(render(Div(sep, Span("a"), sep)), `<div><span>/</span>\n<span>a</span>\n<span>/</span></div>`);
    });
});
// ------------------------------------
// Opt-out
...
//# sourceMappingURL=dev-checks.test.js.map