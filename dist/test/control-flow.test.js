...
describe("Control Flow - Conditionals", () => {
    ...
    it("IfThen with 0 passes through (not swallowed)", () => { assert.strictEqual(render(IfThen(0, (n) => Span(`${n}`))), `<span>0</span>`); });
    it("IfThen with empty string passes through (not swallowed)", () => { assert.strictEqual(render(IfThen("", (s) => Span(`[${s}]`))), `<span>[]</span>`); });
    it("IfThen with NaN passes through (not swallowed)", () => { assert.strictEqual(render(IfThen(NaN, (n) => Span(`${n}`))), `<span>NaN</span>`); });
    it("IfThenElse with 0 takes then-branch", () => { assert.strictEqual(render(IfThenElse(0, (n) => Span(`${n}`), () => Span("none"))), `<span>0</span>`); });
    it("IfThenElse with empty string takes then-branch", () => { assert.strictEqual(render(IfThenElse("", (s) => Span(`[${s}]`), () => Span("none"))), `<span>[]</span>`); });
    it("IfThenElse with NaN takes then-branch", () => { assert.strictEqual(render(IfThenElse(NaN, (n) => Span(`${n}`), () => Span("none"))), `<span>NaN</span>`); });
});
// ------------------------------------
// Control Flow - Match
// ------------------------------------
...
describe("Control Flow - ForEach", () => {
    ...
    it("ForEachElse empty renders the fallback (thunk)", () => {
        assert.strictEqual(render(Ul(ForEachElse([], item => Li(item), () => Li("None")))), `<ul><li>None</li></ul>`);
    });
    it("ForEachElse empty renders the fallback (plain view)", () => {
        assert.strictEqual(render(Ul(ForEachElse([], item => Li(item), Li("Empty")))), `<ul><li>Empty</li></ul>`);
    });
...
describe("Control Flow - Match prototype-chain hardening", () => {
    it("value form falls back to default for Object.prototype keys", () => {
        for (const k of ["toString", "constructor", "valueOf", "hasOwnProperty", "__proto__"]) {
            assert.strictEqual(render(Match(k, { a: () => Span("A") }, () => Span("DEFAULT"))), "<span>DEFAULT</span>", `value key ${k}`);
        }
    });
    it("DU form falls back to default for Object.prototype discriminants", () => {
        for (const k of ["toString", "constructor", "valueOf"]) {
            assert.strictEqual(render(Match({ status: k }, "status", { a: () => Span("A") }, () => Span("DEFAULT"))), "<span>DEFAULT</span>", `DU discriminant ${k}`);
        }
    });
