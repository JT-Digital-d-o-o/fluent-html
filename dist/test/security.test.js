...
import { render, Div, P, Span, Script, Style, Button, A, Img, Form, ObjectEl, Video, Blockquote, sanitizeUrl, } from "../src/index.js";
...
describe("Behavior injection prevention (v4: attributes-only, no JS emission)", () => {
    ...
    it("class options reject non-token payloads at render (injection can't even ride the wire)", () => {
        assert.throws(() => render(Button("x").behavior("toggleClass", { target: ids.panel, class: "x'); alert(1); ('" })), /single CSS class token/);
    });
    it("rejects a raw string smuggled where an Id is required", () => {
        assert.throws(() => render(Button("x").behavior("toggle", { target: "a'b" })), /expects an Id/);
    });
...
describe("hx-status key injection prevention", () => {
    ...
    it("throws at render on a malformed status key (attribute-name injection)", () => {
        const evilOpts = { status: { "404 onload=alert(1)": "swap:none" } };
...
describe("URL scheme sanitization on typed setters (XSS-1)", () => {
    ...
    it("still escapes attribute breakout inside an allowed value", () => {
        assert.strictEqual(render(A("x").setHref(assetUrl('/a"><img src=x onerror=alert(1)>'))), `<a href="/a&quot;&gt;&lt;img src=x onerror=alert(1)&gt;">x</a>`);
    });
