...
import { Div, P, Span, A, Nav, Img, Raw, render, paraView, unfoldView, hyloView, foldView, renderAlgebra, countAlgebra, textAlgebra, ariaDescribeAlgebra, linkedTocCoalgebra, } from "../src/index.js";
// ─── Paramorphism ────────────────────────────────────────────────
describe("paraView", () => {
...
describe("ariaDescribeAlgebra", () => {
    ...
    it("describes nested structure", () => {
        const desc = paraView(ariaDescribeAlgebra, Nav(A("Home").setHref("/"), A("About").setHref("/about")));
...
describe("unfoldView", () => {
    ...
    it("unfolds a list", () => {
        const coalg = (seed) => {
            if (seed.type === "item") {
                return { type: "tag", element: "li", children: [{ type: "list", items: [] }] };
            }
            if (seed.items.length === 0)
                return { type: "text", value: "" };
            return {
                type: "list",
                items: seed.items.map(text => ({ type: "item", text })),
            };
        };
...
describe("linkedTocCoalgebra", () => {
    it("builds a TOC from flat headings", () => {
        ...
        const view = unfoldView(linkedTocCoalgebra, { type: "list", entries: headings });
        const html = render(view);
    ...
    it("builds empty list", () => {
        const view = unfoldView(linkedTocCoalgebra, { type: "list", entries: [] });
        const html = render(view);
        assert.ok(html.includes("<ul>"));
        assert.ok(html.includes("</ul>"));
    });
...
describe("hyloView", () => {
    ...
    it("works with text algebra", () => {
        const coalg = (seed) => {
            ...
            return { type: "tag", element: "p", children: ["text"] };
        };
        assert.equal(hyloView(coalg, textAlgebra, "root"), "Hello\n");
    });
    it("handles raw in hylo", () => {
        const coalg = () => ({ type: "raw", html: "<b>bold</b>" });
    ...
    it("handles list seeds", () => {
        const coalg = (seed) => {
            ...
            return {
                type: "list",
                items: Array.from({ length: seed.n }, (_, i) => ({ type: "item", i })),
            };
        };
    ...
    it("hylo with TOC coalgebra matches unfold-then-fold", () => {
        ...
        const seed = { type: "list", entries: headings };
...
//# sourceMappingURL=recursion-schemes.js.map