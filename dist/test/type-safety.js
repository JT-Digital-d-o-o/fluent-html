...
import { render, Raw, Div, Span, Input, Textarea, Button, Form, A, Area, isTag, isRawString, defineRoutes, IfThen, IfThenElse, Match, MatchValue, Empty, } from "../src/index.js";
...
describe("InputTag typed setters", () => {
    it("setType renders typed value", () => {
        assert.strictEqual(render(Input().setType("email")), '<input type="email">');
    });
    it("setType accepts all standard types", () => {
        for (const t of ["text", "password", "checkbox", "radio", "file", "hidden", "submit", "date", "color", "range"]) {
            assert.ok(render(Input().setType(t)).includes(`type="${t}"`), `type="${t}" should render`);
        }
    });
    it("setAutocomplete renders typed value", () => {
        assert.strictEqual(render(Input().setAutocomplete("email")), '<input autocomplete="email">');
    });
...
describe("ButtonTag typed setters", () => {
    it("setFormmethod renders get", () => {
        assert.strictEqual(render(Button("Go").setFormmethod("get")), '<button formmethod="get">Go</button>');
    });
    it("setFormmethod renders post", () => {
        assert.strictEqual(render(Button("Go").setFormmethod("post")), '<button formmethod="post">Go</button>');
    });
});
describe("FormTag typed setters", () => {
    it("setMethod renders typed value", () => {
        assert.strictEqual(render(Form().setMethod("post").setAction("/submit")), '<form action="/submit" method="post"></form>');
    });
    it("setMethod accepts dialog", () => {
        assert.ok(render(Form().setMethod("dialog")).includes('method="dialog"'));
    });
    it("setTarget renders typed value", () => {
        assert.strictEqual(render(Form().setTarget("_blank")), '<form target="_blank"></form>');
    });
...
describe("defineRoutes typed query params", () => {
    const r = defineRoutes({
        list: { path: "/products", query: { page: "number", sort: ["asc", "desc"] } },
        detail: { path: "/products/:id", query: { tab: ["specs", "reviews"] } },
        plain: { path: "/plain" },
    });
...
describe("Tailwind variant fidelity", () => {
    it("rejects the non-existent only-child variant, accepts only / only-of-type", () => {
        render(Div().variant("only", { mt: "2" }));
        render(Div().variant("only-of-type", { mt: "2" }));
        // @ts-expect-error — "only-child" is not a Tailwind variant; use "only"
        render(Div().variant("only-child", { mt: "2" }));
    });
});
// -------------------------------------------------------
...
describe("type honesty — types that used to lie", () => {
    ...
    it("MatchValue exhaustive form rejects a widened string (returned undefined-as-R before)", () => {
        ...
        assert.strictEqual(MatchValue("a", { a: "1", b: "2" }), "1");
    });
    it("HxSwap accepts arbitrary delays + ignoreTitle, rejects typos (closed union)", () => {
        for (const s of ["innerHTML settle:250ms", "outerHTML swap:1.5s", "outerHTML ignoreTitle:true", "outerHTML scroll:top swap:500ms"]) {
