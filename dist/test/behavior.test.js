...
import { EVENT_TABLE, HTMX_EVENTS, LISTEN_SET, BUILTIN_SPECS, BUILTIN_NAMES, registerBehavior, sealBehaviorRegistry, getBehaviorSpec, allBehaviors, registryHash, decodeOptions, verbPrefix, optionAttr, readAssetStamp, assertBehaviorRuntimeAsset, behaviorStamp, } from "../src/behaviors/index.js";
...
const ids = defineIds(["panel", "second", "banner", "mobile-menu", "menu-backdrop", "cancel-entry", "search"]);
...
describe("behavior() emission grammar", () => {
    ...
    it("toggle — Id[] multi-target joins with spaces; display + force serialize", () => {
        const html = render(Button("X").behavior("toggle", { target: [ids.panel, ids.second], display: "flex", force: false }));
    ...
    it("toggleClass — camelCase verb kebabs in the attribute prefix", () => {
        ...
        assert.equal(html, `<button data-behavior="toggleClass" data-behavior-toggle-class-target="panel" data-behavior-toggle-class-class="opacity-50">Fade</button>`);
    });
    it("drawer — the full composite emits flat options (design doc example)", () => {
        const html = render(Button("Menu")
    ...
    it("clipboard — nested feedback group flattens with hyphens", () => {
        const html = render(Button("Copy invite link").behavior("clipboard", {
            path: "/invite/8f3k",
    ...
    it("onEscape — enum options serialize as literals (design doc example)", () => {
        const html = render(Form().behavior("onEscape", { action: "click", target: ids.cancelEntry, scope: "self" }));
    ...
    it("two behaviors on one element — declaration order preserved in data-behavior", () => {
        const html = render(Button("+386 40 123 456")
    ...
    it("extension namespace — kebabs with a double hyphen (jt:listboxNav)", () => {
        ...
        const html = render(Input().behavior("jt:listboxNav", { list: createId("ac-results"), itemSelector: "[role=option]" }));
...
describe("behavior() render-time guards", () => {
    it("throws on an unknown verb", () => {
        ...
        assert.throws(() => Button("X").behavior("openDialog", { target: ids.panel }), /unknown behavior/);
    });
    it("throws on duplicate same-verb on one element", () => {
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel }).behavior("toggle", { target: ids.second }), /duplicate verb/);
    });
    it("throws on an unknown option", () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- deliberately bypassing the typed surface
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel, foo: 1 }), /unknown option "foo"/);
    });
    it("throws on option type mismatches", () => {
        ...
        assert.throws(() => Button("X").behavior("toggle", { target: "panel" }), /expects an Id/);
        assert.throws(() => Button("X").behavior("toggleClass", { target: ids.panel, class: 5 }), /single CSS class token/);
        assert.throws(() => Button("X").behavior("remove", { target: ids.panel, animateOutTimeoutMs: "x" }), /finite number/);
        assert.throws(() => Button("X").behavior("drawer", { target: ids.panel, closeOn: ["escape", "poll"] }), /expects a list of/);
        assert.throws(() => Button("X").behavior("onEscape", { action: "explode" }), /expects one of/);
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel, event: "keyup" }), /expects one of/);
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel, force: "true" }), /expects a boolean/);
        /* eslint-enable @typescript-eslint/no-explicit-any */
    });
    it("prototype keys never satisfy closed sets (event enum, option lookup)", () => {
        /* eslint-disable @typescript-eslint/no-explicit-any -- deliberately bypassing the typed surface */
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel, event: "constructor" }), /expects one of/);
        assert.throws(() => Button("X").behavior("toggle", { target: ids.panel, constructor: "x" }), /unknown option "constructor"/);
        /* eslint-enable @typescript-eslint/no-explicit-any */
    });
...
describe("registerBehavior", () => {
    ...
    it("throws on built-in override", () => {
        ...
        assert.throws(() => registerBehavior("toggle", spec), /built-in/);
    });
    ...
    it("throws without fixtures — a verb without coverage cannot register (ADR-12)", () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- deliberately invalid
        assert.throws(() => registerBehavior("jt:bare", { options: {}, events: ["click"], fixtures: [] }), /fixture/);
    });
    it("throws on unknown events — including prototype keys", () => {
        ...
        assert.throws(() => registerBehavior("jt:odd", { ...spec, events: ["keyup"] }), /unknown event/);
        assert.throws(() => registerBehavior("jt:odd", { ...spec, events: ["constructor"] }), /unknown event/);
        /* eslint-enable @typescript-eslint/no-explicit-any */
    });
    it("throws after seal; rendering an unregistered verb still throws", () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- deliberately bypassing the typed surface
        assert.throws(() => Button("X").behavior("jt:late"), /unknown behavior/);
    });
    ...
    it("every built-in has ≥ 1 fixture and every fixture emits cleanly", () => {
        for (const { name, fixtures } of allBehaviors()) {
            ...
            for (const fixture of fixtures) {
                ...
                assert.doesNotThrow(() => render(Div().behavior(name, fixture)), `${name} fixture failed to emit`);
            }
        }
    });
    it("all 10 built-ins exist — the ADR-09 vocabulary, nothing else", () => {
        assert.deepEqual([...BUILTIN_NAMES].sort(), ["back", "clipboard", "drawer", "focus", "onClickOutside", "onEscape", "remove", "resetOnSuccess", "toggle", "toggleClass"]);
    });
    it(".hxOn is gone", () => {
        assert.equal(Tag.prototype["hxOn"], undefined);
    });
});
