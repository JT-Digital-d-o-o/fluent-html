...
describe("Interactive Elements", () => {
    ...
    it("Details open", () => {
        assert.strictEqual(render(Details([
            Summary("Expanded"),
            P("Visible content"),
        ]).toggle("open")), `<details open><summary>Expanded</summary>\n<p>Visible content</p></details>`);
    });
...
describe("Media Elements", () => {
    ...
    it("Video with multiple sources", () => {
        ...
        ]).toggle("controls")), `<video controls><source src="video.webm" type="video/webm">\n<source src="video.mp4" type="video/mp4">\nYour browser does not support video.</video>`);
    });
    it("Video with all options", () => {
        assert.strictEqual(render(Video()
            ...
            .setPreload("metadata")
            .toggle("playsinline")), `<video src="video.mp4" poster="poster.jpg" preload="metadata" controls autoplay loop muted playsinline></video>`);
    });
    ...
    it("Audio with preload", () => {
        assert.strictEqual(render(Audio()
            .setSrc("audio.mp3")
            .toggle("controls")
            .setPreload("none")), `<audio src="audio.mp3" preload="none" controls></audio>`);
    });
    it("Video with Track", () => {
        assert.strictEqual(render(Video([
            Source().setSrc("video.mp4").setType("video/mp4"),
...
describe("SVG", () => {
    it("Basic SVG", () => {
        assert.strictEqual(render(Svg(Circle().setCx("50").setCy("50").setR("40").setFill("red")).setWidth("100").setHeight("100").setViewBox("0 0 100 100")), `<svg width="100" height="100" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" fill="red"></circle></svg>`);
    });
    it("SVG with path", () => {
        assert.strictEqual(render(Svg(Path().setD("M10 10 H 90 V 90 H 10 Z").setFill("none").setStroke("black")).setViewBox("0 0 100 100")), `<svg viewBox="0 0 100 100"><path d="M10 10 H 90 V 90 H 10 Z" fill="none" stroke="black"></path></svg>`);
    });
    it("SVG with group", () => {
        assert.strictEqual(render(Svg(G(Rect().setWidth("50").setHeight("50"), Line().setX1("0").setY1("0").setX2("50").setY2("50")).addAttribute("transform", "translate(10, 10)")).setViewBox("0 0 100 100")), `<svg viewBox="0 0 100 100"><g transform="translate(10, 10)"><rect width="50" height="50"></rect>\n<line x1="0" y1="0" x2="50" y2="50"></line></g></svg>`);
    });
    it("SvgTag setStrokeWidth", () => {
        assert.strictEqual(render(Svg(Path().setD("M0 0 L24 24").setStroke("currentColor")).setXmlns().setFill("none").setViewBox("0 0 24 24").setStrokeWidth("1.5")), `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke-width="1.5"><path d="M0 0 L24 24" stroke="currentColor"></path></svg>`);
    });
    it("Circle typed setters", () => {
        assert.strictEqual(render(Circle().setCx("50").setCy("50").setR("25").setFill("blue").setStroke("black").setStrokeWidth("2")), `<circle cx="50" cy="50" r="25" fill="blue" stroke="black" stroke-width="2"></circle>`);
    });
    it("Rect typed setters", () => {
        assert.strictEqual(render(Rect().setX("10").setY("10").setWidth("80").setHeight("40").setRx("5").setRy("5")), `<rect x="10" y="10" width="80" height="40" rx="5" ry="5"></rect>`);
    });
    it("Line typed setters", () => {
        assert.strictEqual(render(Line().setX1("0").setY1("0").setX2("100").setY2("100").setStroke("red").setStrokeLinecap("round")), `<line x1="0" y1="0" x2="100" y2="100" stroke="red" stroke-linecap="round"></line>`);
    });
    it("Path typed setters", () => {
        assert.strictEqual(render(Path().setD("M0 0 L50 50").setFill("none").setStroke("green").setFillRule("evenodd")), `<path d="M0 0 L50 50" fill-rule="evenodd" fill="none" stroke="green"></path>`);
    });
    it("Ellipse typed setters", () => {
        assert.strictEqual(render(Ellipse().setCx("50").setCy("50").setRx("30").setRy("20").setFill("orange")), `<ellipse cx="50" cy="50" rx="30" ry="20" fill="orange"></ellipse>`);
    });
    it("Polygon typed setters", () => {
        assert.strictEqual(render(Polygon().setPoints("50,0 100,100 0,100").setFill("purple")), `<polygon points="50,0 100,100 0,100" fill="purple"></polygon>`);
    });
    it("Polyline typed setters", () => {
        assert.strictEqual(render(Polyline().setPoints("0,0 50,50 100,0").setStroke("black").setFill("none")), `<polyline points="0,0 50,50 100,0" fill="none" stroke="black"></polyline>`);
    });
    it("Text typed setters", () => {
        assert.strictEqual(render(Text("Hello").setX("10").setY("20").setTextAnchor("middle").setFontSize("16")), `<text x="10" y="20" text-anchor="middle" font-size="16">Hello</text>`);
    });
    it("Tspan typed setters", () => {
        assert.strictEqual(render(Tspan("world").setDx("5").setDy("10")), `<tspan dx="5" dy="10">world</tspan>`);
    });
    it("Use typed setters", () => {
        assert.strictEqual(render(Use().setHref("#icon").setX("10").setY("20").setWidth("24").setHeight("24")), `<use href="#icon" x="10" y="20" width="24" height="24"></use>`);
    });
    it("SvgShapeTag shared setters", () => {
        assert.strictEqual(render(Circle().setStrokeLinejoin("bevel").setStrokeDasharray("5 3").setTransform("rotate(45)")), `<circle stroke-linejoin="bevel" stroke-dasharray="5 3" transform="rotate(45)"></circle>`);
    });
    it("setSvgOpacity uses addAttribute", () => {
        assert.strictEqual(render(Rect().setSvgOpacity("0.5")), `<rect opacity="0.5"></rect>`);
    });
});
// ------------------------------------
// Embedded Content
