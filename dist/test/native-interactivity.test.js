...
const ids = defineIds(["user-menu", "dialog"]);
// ---------------------------------------------------------------------------
...
describe("Popover API", () => {
    ...
    it("invoker: setPopovertarget takes an Id and renders the raw id", () => {
        assert.equal(render(Button("Account").setPopovertarget(ids.userMenu)), `<button popovertarget="user-menu">Account</button>`);
    });
    it("setPopovertargetaction emits the action", () => {
        assert.equal(render(Button("Open").setPopovertarget(ids.userMenu).setPopovertargetaction("show")), `<button popovertarget="user-menu" popovertargetaction="show">Open</button>`);
    });
    it("setPopovertargetaction() with no arg omits the attribute (native toggle default)", () => {
        assert.equal(render(Button("Toggle").setPopovertarget(ids.userMenu).setPopovertargetaction()), `<button popovertarget="user-menu">Toggle</button>`);
    });
    it("popover + id on one element", () => {
        assert.equal(render(Div("items").setId(ids.userMenu).setPopover()), `<div id="user-menu" popover="auto">items</div>`);
    });
});
describe("Invoker Commands (<button>)", () => {
    it("show-modal + commandfor (JS-free, no hx-on)", () => {
        assert.equal(render(Button("Edit").setCommand("show-modal").setCommandfor(ids.dialog)), `<button command="show-modal" commandfor="dialog">Edit</button>`);
    });
    it("close", () => {
        assert.equal(render(Button("Done").setCommand("close").setCommandfor(ids.dialog)), `<button command="close" commandfor="dialog">Done</button>`);
    });
    it("author command (--name) is allowed", () => {
        assert.equal(render(Button("Go").setCommand("--my-cmd").setCommandfor(ids.dialog)), `<button command="--my-cmd" commandfor="dialog">Go</button>`);
    });
});
describe("Dialog light-dismiss + formmethod (B-010 completion)", () => {
    ...
    it("closedby + id + open render in schema order", () => {
        assert.equal(render(Dialog().setId(ids.dialog).setClosedby("closerequest").toggle("open")), `<dialog id="dialog" closedby="closerequest" open></dialog>`);
    });
    it("a submit button closes an ancestor <dialog> via formmethod=dialog", () => {
        assert.equal(render(Button("Cancel").setType("submit").setFormmethod("dialog")), `<button type="submit" formmethod="dialog">Cancel</button>`);
    });
});
describe("CSS anchor positioning", () => {
    it("anchorName emits the [anchor-name:--id] class", () => {
        assert.equal(render(A("Account").setPopovertarget(ids.userMenu).anchorName(ids.userMenu)), `<a class="[anchor-name:--user-menu]" popovertarget="user-menu">Account</a>`);
    });
    it("positionAnchor + positionArea place a popover against its anchor", () => {
        assert.equal(render(Div("menu").setId(ids.userMenu).setPopover().positionAnchor(ids.userMenu).positionArea("bottom")), `<div id="user-menu" class="[position-anchor:--user-menu] position-area-bottom" popover="auto">menu</div>`);
    });
...
//# sourceMappingURL=native-interactivity.test.js.map