 ...
 */
export function verbPrefix(verb) {
...
export function optionAttr(verb, optionPath) {
 ...
 */
export function decodeOptions(verb, options, get) {
    const out = {};
    for (const path of Object.keys(options)) {
        ...
        if (type === "number")
            ...
            value = raw === "true";
        else if (type === "id-list" || type === "string-list")
            value = raw.split(" ");
        else if (typeof type === "object" && type.list)
            value = raw.split(" ");
