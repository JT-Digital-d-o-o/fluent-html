...
const COMPOUND_ASSIGN_OPERATORS = new Set([
...
function isStatementPosition(expr) {
...
function destructuredField(node) {
    ...
    if (!owner || !(Node.isVariableDeclaration(owner) || Node.isParameterDeclaration(owner)))
        return null;
    ...
    if (!source || !typeIsTag(source.getType()))
        return null;
    return name;
}
        ...
        if (accessor !== undefined) {
            ...
            if (narrowed && !optional) {
                if (accessor.fallback !== undefined) {
                    const isCallArgument = Node.isCallExpression(parent) && parent.getArguments().includes(node);
...
function run() {
    ...
    if (!dry)
        project.saveSync();
    console.log(`codemod:storage-fields ${dry ? "(dry run) " : ""}— ${totalEdits} site(s) rewritten across ${files.length} file(s); ${totalSkips} skipped for manual review.`);
}
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href)
    run();
//# sourceMappingURL=storage-fields.js.map