...
export function computeVocabCoverage(design) {
    const exact = new Set();
                ...
                for (const args of def.samples ?? []) {
                    for (const cls of emitClasses(s, args)) {
                        for (const cand of design.parseCandidate(cls)) {
                            if (cand.kind === "static")
                                exact.add(cand.root);
                            else if ("root" in cand && typeof cand.root === "string")
                                prefixes.add(cand.root);
                        }
                    }
                }
                break;
