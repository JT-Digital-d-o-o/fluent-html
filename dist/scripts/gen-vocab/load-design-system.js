...
function tailwindRootDir() {
    return path.dirname(requireFromHere.resolve("tailwindcss/package.json"));
}
function installedTailwindVersion() {
    const pkg = JSON.parse(readFileSync(path.join(tailwindRootDir(), "package.json"), "utf8"));
...
export async function loadDesignSystem(css = '@import "tailwindcss";') {
    ...
    if (tailwindVersion !== PINNED_TAILWIND_VERSION) {
        throw new Error(`gen-vocab: installed tailwindcss@${tailwindVersion} ≠ pinned ${PINNED_TAILWIND_VERSION}. ` +
...
//# sourceMappingURL=load-design-system.js.map