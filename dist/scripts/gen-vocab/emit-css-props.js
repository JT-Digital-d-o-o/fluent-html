...
export function cssPropsGeneratedPath() {
...
function tsVersion() {
    return requireFromHere("typescript/package.json").version;
}
...
export function cssPropertyNames() {
    ...
    if (start === -1)
        throw new Error("emit-css-props: CSSStyleDeclaration not found in lib.dom.d.ts");
    const body = libDom.slice(start, libDom.indexOf("\n}", start));
    const names = new Set();
    for (const m of body.matchAll(/^\s+([a-zA-Z][a-zA-Z0-9]*): string;$/gm)) {
        const prop = m[1];
        if (prop === "cssText")
            continue; // serialization accessor, not a property
        if (/^(webkit|moz|ms|o)[A-Z]/.test(prop))
            continue; // deprecated vendor aliases
...
export function renderCssPropsGen() {
...
//# sourceMappingURL=emit-css-props.js.map