 ...
 *
 *     @@UNION <TypeName> <method>[.<group>][ +arb]@@
 *
 ...
 * escape-hatch arm). A merged method's per-family lists are addressed as
 * `method.group` (`text.align` → the `align` group of the `text` row). A row
 * or group that is missing, or whose spec is not `literals`, is a loud error —
 * the generator never invents members.
 *
...
export function templateUnions() {
    const template = readFileSync(templatePath(), "utf8");
    const unions = [];
    for (const line of template.split("\n")) {
        const m = MARKER.exec(line);
        if (m)
            unions.push({ typeName: m[1], method: m[2], arbitrary: m[3] !== undefined });
    }
    return unions;
}
/**
 ...
 */
export function literalsOf(address) {
    const [method, groupName, ...extra] = address.split(".");
    if (extra.length > 0)
        throw new Error(`gen-vocab: malformed union address "${address}" (expected method or method.group)`);
    const row = classVocab.find((d) => d.method === method);
    if (!row)
        throw new Error(`gen-vocab: template references unknown vocab method "${method}"`);
    let spec = row.values;
    if (groupName !== undefined) {
        if (spec?.kind !== "group") {
            throw new Error(`gen-vocab: vocab row "${method}" has no value groups (found: ${spec?.kind ?? "none"}) — address "${address}"`);
        }
        spec = spec.groups[groupName];
        if (!spec)
            throw new Error(`gen-vocab: vocab row "${method}" has no group "${groupName}"`);
    }
    if (spec?.kind !== "literals") {
        throw new Error(`gen-vocab: vocab address "${address}" has no literals values spec (found: ${spec?.kind ?? "none"})`);
    }
    return spec.list;
}
/** Render the full content of `tailwind-types.gen.ts`. */
export function renderTailwindTypesGen() {
    const header = [
